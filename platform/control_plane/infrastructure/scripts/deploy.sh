...
if terraform apply tfplan; then
    ...
    # Show CodeCommit info if enabled
    CODECOMMIT_REPO=$(terraform output -raw codecommit_repository_name 2>/dev/null || echo "")
    if [ -n "$CODECOMMIT_REPO" ] && [ "$CODECOMMIT_REPO" != "" ]; then
        echo "CodeCommit Repository:"
        terraform output -raw codecommit_clone_url_http
        echo
        echo
        echo -e "${GREEN}✓ CodeCommit repository created${NC}"
        echo
        echo -e "${BLUE}To use CodeCommit for Git-based deployments:${NC}"
        echo
        echo "1. Configure git credential helper:"
        ...
        echo "   git clone $(terraform output -raw codecommit_clone_url_http)"
        echo
    ...
    if [ -z "$CODECOMMIT_REPO" ] || [ "$CODECOMMIT_REPO" = "" ]; then
        echo "4. (Optional) Enable CodeCommit by setting enable_codecommit=true in terraform.tfvars"
    fi
    echo
