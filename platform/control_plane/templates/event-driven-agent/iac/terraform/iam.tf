resource "aws_iam_role" "runtime" {
  ...
  description = "IAM role assumed by Bedrock AgentCore for the event-driven agent."
...
# S3 Read Access (conditional on event_bucket_name)
resource "aws_iam_policy" "s3_read" {
  count = var.event_bucket_name != "" ? 1 : 0
    ...
    Statement = [
      {
        ...
        Resource = [
          "arn:aws:s3:::${var.event_bucket_name}",
          "arn:aws:s3:::${var.event_bucket_name}/*"
        ]
      },
      {
        ...
        Effect   = "Allow"
        Action   = "s3:PutObject"
        Resource = "arn:aws:s3:::${var.event_bucket_name}/results/*"
      }
    ]
  })
