...
docker build -t ${PROJECT_NAME}-rag:latest .
...
if [ "$CONFIRM" = "yes" ]; then
  ...
  echo ""
  echo "Next steps:"
  echo "1. Upload documents to S3 bucket for ingestion"
  echo "2. Configure vector store credentials"
  echo "3. Run ./test.sh to verify deployment"
