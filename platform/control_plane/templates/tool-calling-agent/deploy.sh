...
# Tool-Calling Agent Deployment Script
...
echo "========================================"
echo "Tool-Calling Agent Deployment"
echo "========================================"
...
# Tag for ECR (TODO: Create ECR repository first)
# AWS_ACCOUNT_ID=$(aws sts get-caller-identity --query Account --output text)
# ECR_REPO="${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/${PROJECT_NAME}"
# docker tag ${PROJECT_NAME}-agent:latest ${ECR_REPO}:latest

# Push to ECR (TODO: Uncomment when ECR is set up)
# echo "Pushing to ECR..."
...
if [ "$CONFIRM" = "yes" ]; then
  ...
  echo ""
  echo "Next steps:"
  echo "1. Set up environment variables (API keys, etc.)"
