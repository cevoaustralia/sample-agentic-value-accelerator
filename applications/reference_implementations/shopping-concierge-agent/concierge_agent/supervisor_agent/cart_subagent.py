...
5. Processing payments via tokenization
...
- `cart_get_payment_config`: Get payment configuration for card entry

IMPORTANT GUIDELINES:

1. Always confirm cart operations with the user
2. For checkout, ensure the user has a payment method onboarded
3. When onboarding cards, explain the secure card tokenization process
...
7. Use tokenization for all payment processing
...
async def cart_manager(query: str, user_id: str = "", session_id: str = ""):
    ...
    - get_payment_config(user_id): Get secure card entry config
