...
The application uses mock payment APIs by default. No external payment credentials are required.

```bash
npm run deploy:frontend
```

The mock payment service simulates the full card onboarding and purchase flow in the browser.
...
|--------|-------------|------|
| `npm run deploy` | Deploy Amplify + MCP + Agent | ~9 min |
...
| `npm run clean` | Delete all resources (frontend, agent, MCP, Amplify) | ~5 min |
