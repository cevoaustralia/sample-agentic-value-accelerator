/**
 * Frontend Mock Payment API Service
 *
 * This service provides mock/fake payment API responses directly in the browser,
 * eliminating the need to run the local backend server during development.
 *
 * Enable by setting: VITE_PAYMENT_MOCK_MODE=true in .env.local
 */
...
export const mockPaymentService = {
   ...
   */
  isEnabled(): boolean {
    return import.meta.env.VITE_PAYMENT_MOCK_MODE === 'true';
  },
    ...
    if (data.step === 'AUTHENTICATE') {
      ...
      return {
        ...
        authenticationContext: {
          endpoint: 'https://mock-payment.local/attestation',
  ...
  async vicEnrollCard(data: {
    ...
    return {
      success: true,
      consumerId: generateMockId('VIC-CONSUMER'),
      clientDeviceId: generateMockId('VIC-DEVICE'),
