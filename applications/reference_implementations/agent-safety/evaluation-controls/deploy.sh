#!/bin/bash
# ============================================================
# Agent Safety — Evaluation Controls Deployment
# ============================================================
# Deploys: Eval Execution Role, Auto Eval Lambda, EventBridge rule.
#
# Usage:
#   ./deploy.sh --profile mbavadiy-Admin --region us-east-2
#   ./deploy.sh --profile mbavadiy-Admin --region us-east-2 \
#     --eval-signals-table safety-dashboard-eval-signals
# ============================================================
set -euo pipefail

REGION="us-east-1"
PROFILE=""
STACK_NAME="agent-safety-eval-controls"
EVAL_SIGNALS_TABLE="evaluation-signals"
REGISTRY_TABLE="agent-registry"
SNS_TOPIC_ARN=""

while [[ $# -gt 0 ]]; do
  case $1 in
    --region) REGION="$2"; shift 2 ;;
    --profile) PROFILE="$2"; shift 2 ;;
    --stack-name) STACK_NAME="$2"; shift 2 ;;
    --eval-signals-table) EVAL_SIGNALS_TABLE="$2"; shift 2 ;;
    --registry-table) REGISTRY_TABLE="$2"; shift 2 ;;
    --sns-topic-arn) SNS_TOPIC_ARN="$2"; shift 2 ;;
    -h|--help)
      echo "Usage: ./deploy.sh --profile <profile> [options]"
      echo "  --region <region>                 AWS region (default: us-east-1)"
      echo "  --eval-signals-table <name>       DynamoDB table (default: evaluation-signals)"
      echo "  --sns-topic-arn <arn>              SNS topic for alarm notifications"
      exit 0 ;;
    *) echo "Unknown arg: $1"; exit 1 ;;
  esac
done

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
AWS_OPTS="--region $REGION"
if [ -n "$PROFILE" ]; then
  AWS_OPTS="$AWS_OPTS --profile $PROFILE"
fi

# Auto-detect SNS topic if not provided
if [ -z "$SNS_TOPIC_ARN" ]; then
  SNS_TOPIC_ARN=$(aws sns list-topics $AWS_OPTS --query "Topics[?contains(TopicArn,'agent-cost-alerts')].TopicArn" --output text 2>/dev/null) || true
fi

echo ""
echo "============================================================"
echo "  Agent Safety — Evaluation Controls Deploy"
echo "============================================================"
echo "  Region:     $REGION"
echo "  Stack:      $STACK_NAME"
echo "  Eval Table: $EVAL_SIGNALS_TABLE"
echo "  SNS Topic:  ${SNS_TOPIC_ARN:-none}"
echo "============================================================"

echo ""
echo "🚀 Deploying CloudFormation stack..."
aws cloudformation deploy \
  --template-file "$SCRIPT_DIR/template.yaml" \
  --stack-name "$STACK_NAME" \
  --capabilities CAPABILITY_NAMED_IAM \
  --parameter-overrides \
    EvalSignalsTableName="$EVAL_SIGNALS_TABLE" \
    RegistryTableName="$REGISTRY_TABLE" \
    SamplingPercentage=100 \
    SNSTopicArn="${SNS_TOPIC_ARN:-}" \
  $AWS_OPTS \
  --no-fail-on-empty-changeset || { echo "❌ Stack failed."; exit 1; }
echo "   ✅ Stack deployed"

# Create boto3 layer for the Lambda
echo ""
echo "📦 Creating boto3 Lambda layer..."
LAYER_DIR=$(mktemp -d)
pip3 install "boto3>=1.42.80" -t "$LAYER_DIR/python" --quiet --upgrade --no-cache-dir 2>/dev/null
(cd "$LAYER_DIR" && zip -r "$LAYER_DIR/layer.zip" python -q)
LAYER_ARN=$(aws lambda publish-layer-version \
  --layer-name "${STACK_NAME}-boto3-agentcore" \
  --description "boto3 with AgentCore evaluation API support" \
  --zip-file "fileb://$LAYER_DIR/layer.zip" \
  --compatible-runtimes python3.11 python3.12 python3.13 \
  $AWS_OPTS --query 'LayerVersionArn' --output text 2>/dev/null) || true
rm -rf "$LAYER_DIR"

if [ -n "$LAYER_ARN" ]; then
  echo "   Layer: $LAYER_ARN"
  aws lambda update-function-configuration \
    --function-name "${STACK_NAME}-AutoEval" \
    --layers "$LAYER_ARN" \
    $AWS_OPTS --no-cli-pager > /dev/null 2>&1 || true
  echo "   ✅ Layer attached"
fi

echo ""
echo "============================================================"
echo "  ✅ Evaluation Controls deployed!"
echo "============================================================"
aws cloudformation describe-stacks --stack-name "$STACK_NAME" $AWS_OPTS \
  --query 'Stacks[0].Outputs[*].[OutputKey,OutputValue]' --output table --no-cli-pager 2>/dev/null
