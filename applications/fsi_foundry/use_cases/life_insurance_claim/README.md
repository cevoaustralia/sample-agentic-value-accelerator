...
```
use_cases/life_insurance_claim/
├── README.md
└── src/
...
| S3 path | `data/samples/life_insurance_claim/{claim_id}/` |
...
```bash
USE_CASE_ID=life_insurance_claim FRAMEWORK=strands AWS_REGION=us-east-1 \
  ./applications/fsi_foundry/scripts/deploy/full/deploy_agentcore.sh
```

This deploys:
- Docker image to ECR
- AgentCore runtime via Terraform
- React UI to S3 + CloudFront (from `ui/life_insurance_claim/`)

### Local Development

```bash
# Run the UI locally
cd applications/fsi_foundry/ui/life_insurance_claim
npm install
npm run dev
```
