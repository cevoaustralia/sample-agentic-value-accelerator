...
import type { ScreeningResponse, ExecutionStatus } from '../types';
...
export default function AgentConsole({ config }: Props) {
  ...
  const [screeningType, setScreeningType] = useState(input_schema.type_options[0].value);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      const payload: Record<string, string> = {
        ...
        [input_schema.type_field]: screeningType,
      };
  ...
  }, [entityId, screeningType, config, input_schema]);
    ...
    <div className="max-w-6xl mx-auto px-6 py-10 space-y-8">
      ...
      <div className="animate-fadeSlideUp">
        ...
        <p className="text-sm mt-1" style={{ color: 'var(--text-muted)' }}>
          Screen entities through the AI media intelligence engine
        </p>
      </div>
      ...
      <div className="card animate-fadeSlideUp stagger-1">
        ...
        <div className="mb-6">
          ...
          <input
            ...
            style={{
              borderColor: entityId ? 'var(--indigo-400)' : '#E5E7EB',
              background: entityId ? 'var(--indigo-50)' : 'white',
            }}
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {input_schema.type_options.map((opt) => {
              const selected = screeningType === opt.value;
              return (
                <button
                  key={opt.value}
                  onClick={() => setScreeningType(opt.value)}
                  className="flex flex-col items-center gap-2 p-4 rounded-xl border-2 transition-all duration-200 cursor-pointer"
                  style={{
                    borderColor: selected ? 'var(--indigo-700)' : '#E5E7EB',
      ...
      {status === 'running' && (
        <div className="card animate-fadeSlideUp">
          <h3 className="text-sm font-bold mb-5" style={{ color: 'var(--charcoal)' }}>Screening Pipeline</h3>
              ...
              <div key={label} className="flex items-center gap-3 flex-1">
                ...
                {i < flowLabels.length - 1 && (
                  <div className="flex-1 h-1 rounded-full mx-2 mt-[-20px]"
                    style={{
                      background: activeStep > i ? 'var(--green-500)' : '#E5E7EB',
          ...
          <div className="grid grid-cols-3 gap-3">
            {config.agents.map((agent, i) => {
              const agentColors = ['#4338CA', '#F97316', '#22C55E'];
              ...
              return (
                <div key={agent.id}
                  ...
                  style={{
                    borderColor: isActive ? agentColors[i] : isDone ? 'var(--green-500)' : '#E5E7EB',
      ...
      {status === 'error' && error && (
        <div className="card animate-fadeSlideUp" style={{ borderLeft: '4px solid #EF4444' }}>
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="text-sm font-bold text-red-700 mb-1">Screening Error</h3>
