...
import type { MemoResponse, ExecutionStatus } from '../types';
...
const typeIcons: Record<string, string> = {
  ...
  DATA_GATHERING: 'M4 7v10c0 2.21 3.582 4 8 4s8-1.79 8-4V7M4 7c0 2.21 3.582 4 8 4s8-1.79 8-4M4 7c0-2.21 3.582-4 8-4s8 1.79 8 4',
  CREDIT_ANALYSIS: 'M3 3v18h18M7 16l4-8 4 4 4-12',
  MEMO_GENERATION: 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z',
};

const flowLabels = ['Gathering Data', 'Analyzing Credit', 'Writing Memo'];
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-10 space-y-8">
      ...
      <div className="animate-fadeSlideUp">
        <h1 className="text-3xl font-extrabold tracking-tight heading-serif" style={{ color: 'var(--charcoal-900)' }}>
          Credit Analysis Console
        </h1>
        <p className="text-sm mt-1" style={{ color: 'var(--text-muted)' }}>
          Generate credit research memos through the AI analysis engine
        </p>
      </div>
      ...
      <div className="card animate-fadeSlideUp stagger-1">
        ...
        <div className="mb-6">
          ...
          <input
            ...
            style={{
              borderColor: entityId ? 'var(--blue)' : '#E2E8F0',
            ...
            {input_schema.type_options.map((opt) => {
              ...
              return (
                <button
                  ...
                  style={{
                    borderColor: selected ? 'var(--charcoal-900)' : '#E2E8F0',
                    background: selected ? 'var(--blue-50)' : 'white',
                    transform: selected ? 'scale(1.02)' : 'scale(1)',
                  }}
                >
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center"
                    style={{ background: selected ? 'linear-gradient(135deg, #18181B, #27272A)' : '#F1F5F9' }}>
                  ...
                  <span className="text-xs font-bold" style={{ color: selected ? 'var(--charcoal-900)' : 'var(--text-secondary)' }}>
                    {opt.label}
                  </span>
                </button>
              );
      ...
      {status === 'running' && (
        <div className="card animate-fadeSlideUp">
          <h3 className="text-sm font-bold mb-5" style={{ color: 'var(--charcoal-900)' }}>Credit Analysis Pipeline</h3>
              ...
              <div key={label} className="flex items-center gap-3 flex-1">
                <div className="flex flex-col items-center">
                  ...
                  <span className="text-xs font-semibold mt-2" style={{
                    color: activeStep >= i ? 'var(--charcoal-900)' : 'var(--text-muted)',
                  }}>{label}</span>
                </div>
                {i < flowLabels.length - 1 && (
                  <div className="flex-1 h-1 rounded-full mx-2 mt-[-20px]"
                    style={{
                      background: activeStep > i ? 'var(--emerald)' : '#E2E8F0',
          ...
          <div className="grid grid-cols-3 gap-3">
            {config.agents.map((agent, i) => {
              const agentColors = ['#2563EB', '#D97706', '#059669'];
              ...
              return (
                <div key={agent.id}
                  ...
                  style={{
                    borderColor: isActive ? agentColors[i] : isDone ? 'var(--emerald)' : '#E2E8F0',
                    background: isActive ? `${agentColors[i]}08` : isDone ? 'var(--emerald-50)' : 'white',
                  }}>
                  <div className="flex items-center gap-2 mb-2">
                    {isDone ? (
                      <div className="w-5 h-5 rounded-full flex items-center justify-center" style={{ background: 'var(--emerald-50)' }}>
                    ...
                    <span className="text-xs font-bold" style={{ color: 'var(--charcoal-900)' }}>{agent.name}</span>
                  </div>
      ...
      {status === 'error' && error && (
        <div className="card animate-fadeSlideUp" style={{ borderLeft: '4px solid #EF4444' }}>
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="text-sm font-bold text-red-700 mb-1">Analysis Error</h3>
