  ...
  while (true) {
    ...
    if (data.status === 'COMPLETE' && data.result) {
      ...
      if (typeof parsed === 'string') {
        try { parsed = JSON.parse(parsed) as GenerationResponse; } catch { /* use as-is */ }
      }
      const raw = parsed as unknown as Record<string, unknown>;
      if (typeof raw.summary === 'string' && raw.summary.startsWith('{')) {
        try { parsed = JSON.parse(raw.summary as string) as GenerationResponse; } catch { /* fall through */ }
      }
      return parsed;
    }
