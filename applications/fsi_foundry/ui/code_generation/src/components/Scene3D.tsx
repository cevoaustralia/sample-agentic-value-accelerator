...
/* ============================================
   GRID SHADER — digital code landscape
   ============================================ */
...
const gridFragmentShader = `
  ...
  void main() {
    ...
    vec3 deepColor = vec3(0.04, 0.02, 0.1);
    vec3 shallowColor = vec3(0.1, 0.05, 0.2);
...
const terrainFragmentShader = `
  ...
  void main() {
    vec3 rockColor = vec3(0.15, 0.08, 0.25);
    vec3 snowColor = vec3(0.6, 0.5, 0.85);
    vec3 midColor = vec3(0.1, 0.06, 0.2);
    vec3 baseColor = vec3(0.05, 0.03, 0.1);
    ...
    color += vec3(0.34, 0.22, 0.96) * pow(rimDot, 4.0) * 0.2;
...
const skyFragmentShader = `
  ...
  void main() {
    ...
    vec3 bottomColor = vec3(0.04, 0.02, 0.08);
    vec3 midColor = vec3(0.06, 0.03, 0.12);
    ...
    // Purple aurora glow on horizon
    float horizonGlow = exp(-pow((y - 0.15) * 8.0, 2.0));
    color += vec3(0.2, 0.1, 0.4) * horizonGlow * 0.4;

    // Blue accent
    float blueGlow = exp(-pow((y - 0.2) * 6.0, 2.0));
    color += vec3(0.1, 0.15, 0.3) * blueGlow * 0.3 * (0.5 + 0.5 * dir.x);
...
export default function Scene3D() {
  ...
  useEffect(() => {
    ...
    scene.fog = new THREE.FogExp2(0x060310, 0.004);
    ...
    const moonLight = new THREE.DirectionalLight(0x6644cc, 0.8);
    moonLight.position.set(20, 40, 30);
    scene.add(moonLight);

    const purpleLight = new THREE.PointLight(0x8B5CF6, 2, 60);
    purpleLight.position.set(-10, 8, 10);
    scene.add(purpleLight);

    const blueLight = new THREE.PointLight(0x60A5FA, 1.5, 50);
    blueLight.position.set(15, 6, 5);
    scene.add(blueLight);
    ...
    function animate() {
      ...
      purpleLight.intensity = 2 + Math.sin(t * 0.8) * 0.5;
      blueLight.intensity = 1.5 + Math.sin(t * 0.6 + 2) * 0.4;
