...
import type { TradingResponse, ExecutionStatus } from '../types';
...
export default function AgentConsole({ config }: Props) {
  ...
  const [result, setResult] = useState<TradingResponse | null>(null);
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-10 space-y-8">
      ...
      <div className="animate-fadeSlideUp">
        <h1 className="text-3xl font-extrabold tracking-tight heading-terminal" style={{ color: 'var(--white)' }}>
          Trading Console
        </h1>
        <p className="text-sm mt-1" style={{ color: 'var(--text-muted)' }}>
          Generate market analysis and trade ideas through the AI trading engine
        </p>
      </div>
      ...
      <div className="card animate-fadeSlideUp stagger-1">
        ...
        <div className="mb-6">
          ...
          <input
            ...
            style={{
              borderColor: entityId ? '#22C55E' : 'var(--border-color)',
              background: entityId ? 'rgba(34,197,94,0.05)' : 'var(--bg-card)',
            ...
            <div className="flex items-center gap-2 mt-2">
              ...
              {input_schema.test_entities.map((id) => (
                <button key={id} onClick={() => setEntityId(id)}
                  className="text-xs font-bold px-2.5 py-1 rounded-lg transition-colors hover:opacity-80 heading-terminal"
            ...
            {input_schema.type_options.map((opt) => {
              ...
              return (
                <button
                  ...
                  style={{
                    borderColor: selected ? '#22C55E' : 'var(--border-color)',
      ...
      {status === 'running' && (
        <div className="card animate-fadeSlideUp">
          <h3 className="text-sm font-bold mb-5" style={{ color: 'var(--white)' }}>Trading Pipeline</h3>
                ...
                <div className="flex flex-col items-center">
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center text-xs font-bold transition-all duration-500 ${
                      activeStep > i ? 'text-white' : activeStep === i ? 'text-white animate-pulseGreen' : ''
                    }`}
                    style={{
                  ...
                  <span className="text-xs font-semibold mt-2" style={{
                    color: activeStep >= i ? 'var(--white)' : 'var(--text-muted)',
                  }}>{label}</span>
                </div>
                {i < flowLabels.length - 1 && (
                  <div className="flex-1 h-1 rounded-full mx-2 mt-[-20px]"
                    style={{
                      background: activeStep > i ? '#22C55E' : 'rgba(100,116,139,0.15)',
          ...
          <div className="grid grid-cols-3 gap-3">
            {config.agents.map((agent, i) => {
              const agentColors = ['#22C55E', '#F59E0B', '#4ADE80'];
              ...
              return (
                <div key={agent.id}
                  ...
                  style={{
                    borderColor: isActive ? agentColors[i] : isDone ? '#22C55E' : 'var(--border-color)',
                    background: isActive ? `${agentColors[i]}08` : isDone ? 'rgba(34,197,94,0.05)' : 'var(--bg-card)',
                  }}>
                  <div className="flex items-center gap-2 mb-2">
                    {isDone ? (
                      ...
                      <div className="w-5 h-5 rounded-full" style={{ background: 'rgba(100,116,139,0.15)' }} />
                    )}
                    <span className="text-xs font-bold" style={{ color: 'var(--white)' }}>{agent.name}</span>
                  </div>
      ...
      {status === 'error' && error && (
        <div className="card animate-fadeSlideUp" style={{ borderLeft: '4px solid #EF4444' }}>
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="text-sm font-bold mb-1" style={{ color: '#F87171' }}>Analysis Error</h3>
