...
import type { InsightsResponse, ExecutionStatus } from '../types';
...
const typeIcons: Record<string, string> = {
  ...
  SIGNAL_GENERATION: 'M22 12h-4l-3 9L9 3l-3 9H2',
  CROSS_ASSET_ANALYSIS: 'M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15',
  SCENARIO_MODELING: 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z',
};

const flowLabels = ['Generating Signals', 'Cross-Asset Scan', 'Modeling Scenarios'];
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-10 space-y-8">
      ...
      <div className="animate-fadeSlideUp">
        <h1 className="text-3xl font-extrabold tracking-tight heading-dash text-white">
          Analysis Console
        </h1>
        <p className="text-sm mt-1" style={{ color: 'var(--text-muted)' }}>
          Generate trading insights through the AI signal intelligence engine
        </p>
      </div>
      ...
      <div className="card animate-fadeSlideUp stagger-1">
        ...
        <div className="mb-6">
          ...
          <input
            ...
            style={{
              borderColor: entityId ? '#06B6D4' : 'var(--dark-600)',
              background: entityId ? 'rgba(6,182,212,0.06)' : 'var(--dark-700)',
              color: 'white',
            }}
            ...
            {input_schema.type_options.map((opt) => {
              ...
              return (
                <button
                  ...
                  style={{
                    borderColor: selected ? '#06B6D4' : 'var(--dark-600)',
                    background: selected ? 'rgba(6,182,212,0.08)' : 'var(--dark-700)',
                    transform: selected ? 'scale(1.02)' : 'scale(1)',
                  }}
                >
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center"
                    style={{ background: selected ? 'linear-gradient(135deg, #06B6D4, #0891B2)' : 'var(--dark-600)' }}>
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none"
                      stroke={selected ? 'white' : '#737373'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                      <path d={typeIcons[opt.value] || typeIcons.FULL} />
                    </svg>
                  </div>
                  <span className="text-xs font-bold" style={{ color: selected ? 'var(--cyan-400)' : 'var(--text-secondary)' }}>
                    {opt.label}
                  </span>
                </button>
              );
      ...
      {status === 'running' && (
        <div className="card animate-fadeSlideUp">
          <h3 className="text-sm font-bold mb-5 text-white">Analysis Pipeline</h3>
              ...
              <div key={label} className="flex items-center gap-3 flex-1">
                    ...
                    style={{
                      background: activeStep > i
                        ...
                        : activeStep === i
                          ...
                          : 'var(--dark-700)',
                      color: activeStep >= i ? 'white' : '#525252',
                    }}
                  >
                    {activeStep > i ? (
                ...
                {i < flowLabels.length - 1 && (
                  <div className="flex-1 h-1 rounded-full mx-2 mt-[-20px]"
                    style={{
                      background: activeStep > i ? '#06B6D4' : 'var(--dark-600)',
          ...
          <div className="grid grid-cols-3 gap-3">
            {config.agents.map((agent, i) => {
              const agentColors = ['#06B6D4', '#D946EF', '#F59E0B'];
              ...
              return (
                <div key={agent.id}
                  ...
                  style={{
                    borderColor: isActive ? agentColors[i] : isDone ? '#06B6D4' : 'var(--dark-600)',
                    background: isActive ? `${agentColors[i]}08` : isDone ? 'rgba(6,182,212,0.06)' : 'var(--dark-800)',
                  }}>
                  <div className="flex items-center gap-2 mb-2">
                    {isDone ? (
                      <div className="w-5 h-5 rounded-full flex items-center justify-center" style={{ background: 'rgba(6,182,212,0.15)' }}>
                        <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#06B6D4" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
                          <polyline points="20 6 9 17 4 12" />
                        </svg>
                      </div>
                    ) : isActive ? (
                      ...
                      <div className="w-5 h-5 rounded-full" style={{ background: 'var(--dark-700)' }} />
                    )}
                    <span className="text-xs font-bold text-white">{agent.name}</span>
                  </div>
                  <div className="h-1.5 rounded-full overflow-hidden" style={{ background: 'var(--dark-700)' }}>
                    <div
      ...
      {status === 'error' && error && (
        <div className="card animate-fadeSlideUp" style={{ borderLeft: '4px solid #EF4444' }}>
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="text-sm font-bold mb-1" style={{ color: '#EF4444' }}>Analysis Error</h3>
