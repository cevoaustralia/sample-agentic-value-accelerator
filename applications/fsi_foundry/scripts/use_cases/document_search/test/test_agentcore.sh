...
# ============================================================================
# AVA - Document Search AgentCore Test Script
# ============================================================================
# Tests the document_search use case deployed on Bedrock AgentCore Runtime.
# Validates runtime invocation with various search types.
#
# Usage:
#   ./test_agentcore.sh [use_case_id]
#   FRAMEWORK=strands ./test_agentcore.sh document_search
# ============================================================================

set +e  # Don't exit on error - continue running all tests

# Use case configuration
USE_CASE_ID="${1:-document_search}"
...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}AgentCore Runtime Test - Document Search${NC}"
...
if [ "$STACK_STATUS" = "NOT_FOUND" ]; then
    ...
    echo "  USE_CASE_ID=document_search FRAMEWORK=$FRAMEWORK AWS_REGION=$AWS_REGION ./applications/fsi_foundry/scripts/deploy/full/deploy_agentcore.sh"
    exit 1
fi
...
# Test 1: Full Document Search
echo -e "${YELLOW}========================================${NC}"
echo -e "${YELLOW}Test 1: Full Document Search${NC}"
echo -e "${YELLOW}========================================${NC}"
echo ""
echo -e "${BLUE}Note: This may take 60-90 seconds as it runs multiple AI agents...${NC}"
echo ""

TEST_PAYLOAD='{"query": "anti-money laundering compliance policy", "document_type": "full"}'
echo "Request:"
echo "${TEST_PAYLOAD}" | jq '.'
echo ""

RESPONSE_FILE="/tmp/agentcore-ds-test1-$.json"
...
if invoke_agentcore "${TEST_PAYLOAD}" "${RESPONSE_FILE}"; then
    ...
    if echo "$RESPONSE_TEXT" | grep -q "'search_id'\|\"search_id\"" && \
       echo "$RESPONSE_TEXT" | grep -q "'summary'\|\"summary\""; then
        echo -e "${GREEN}✓ Full document search PASSED${NC}"
        ((TESTS_PASSED++))
    else
        echo -e "${RED}✗ Full document search FAILED (Invalid response)${NC}"
        ((TESTS_FAILED++))
    fi
else
    echo -e "${RED}✗ Full document search FAILED (Invocation error)${NC}"
    ((TESTS_FAILED++))
fi
rm -f ${RESPONSE_FILE}
echo ""
...
echo -e "${YELLOW}========================================${NC}"
echo -e "${YELLOW}Test 2: Policy Search${NC}"
echo -e "${YELLOW}========================================${NC}"
echo ""

TEST_PAYLOAD='{"query": "customer due diligence requirements", "document_type": "policy"}'
echo "Request:"
echo "${TEST_PAYLOAD}" | jq '.'
echo ""

RESPONSE_FILE="/tmp/agentcore-ds-test2-$.json"
...
if invoke_agentcore "${TEST_PAYLOAD}" "${RESPONSE_FILE}"; then
    ...
    RESPONSE_TEXT=$(cat ${RESPONSE_FILE})
    if echo "$RESPONSE_TEXT" | grep -q "'search_id'\|\"search_id\""; then
        echo -e "${GREEN}✓ Policy search PASSED${NC}"
        ((TESTS_PASSED++))
    else
        echo -e "${RED}✗ Policy search FAILED (Invalid response)${NC}"
        ((TESTS_FAILED++))
    fi
else
    echo -e "${RED}✗ Policy search FAILED (Invocation error)${NC}"
    ((TESTS_FAILED++))
fi
rm -f ${RESPONSE_FILE}
echo ""

# Test 3: Compliance Search with Context
echo -e "${YELLOW}========================================${NC}"
echo -e "${YELLOW}Test 3: Compliance Search with Context${NC}"
echo -e "${YELLOW}========================================${NC}"
echo ""

TEST_PAYLOAD='{"query": "transaction monitoring procedures", "document_type": "compliance", "additional_context": "Focus on suspicious activity reporting"}'
echo "Request:"
echo "${TEST_PAYLOAD}" | jq '.'
echo ""

RESPONSE_FILE="/tmp/agentcore-ds-test3-$.json"
...
if invoke_agentcore "${TEST_PAYLOAD}" "${RESPONSE_FILE}"; then
    ...
    RESPONSE_TEXT=$(cat ${RESPONSE_FILE})
    if echo "$RESPONSE_TEXT" | grep -q "'search_id'\|\"search_id\""; then
        echo -e "${GREEN}✓ Compliance search PASSED${NC}"
        ((TESTS_PASSED++))
    else
        echo -e "${RED}✗ Compliance search FAILED (Invalid response)${NC}"
        ((TESTS_FAILED++))
    fi
else
    echo -e "${RED}✗ Compliance search FAILED (Invocation error)${NC}"
    ((TESTS_FAILED++))
fi
rm -f ${RESPONSE_FILE}
echo ""

# Test 4: Edge Case - Minimal Query
echo -e "${YELLOW}========================================${NC}"
echo -e "${YELLOW}Test 4: Edge Case - Minimal Query${NC}"
echo -e "${YELLOW}========================================${NC}"
echo ""

TEST_PAYLOAD='{"query": "xyz nonexistent topic", "document_type": "full"}'
echo "Request:"
echo "${TEST_PAYLOAD}" | jq '.'
echo ""

RESPONSE_FILE="/tmp/agentcore-ds-test4-$.json"
...
if invoke_agentcore "${TEST_PAYLOAD}" "${RESPONSE_FILE}"; then
    ...
    if echo "$RESPONSE_TEXT" | grep -qi "error\|not found\|No data found"; then
        ...
        echo -e "${YELLOW}⚠ Edge case handling PASSED (graceful handling)${NC}"
        ((TESTS_PASSED++))
    fi
else
    echo -e "${GREEN}✓ Edge case handling PASSED (error returned)${NC}"
    ((TESTS_PASSED++))
fi
rm -f ${RESPONSE_FILE}
echo ""
...
echo -e "${YELLOW}========================================${NC}"
echo ""
echo -e "${BLUE}Note: Running 5 concurrent policy searches...${NC}"
echo ""
...
for i in {1..5}; do
    (
        PAYLOAD="{\"query\": \"banking policy $i\", \"document_type\": \"policy\"}"
        PAYLOAD_BASE64=$(echo -n "${PAYLOAD}" | base64)
        TEMP_FILE="/tmp/agentcore-ds-load-$i-$.json"
        aws bedrock-agentcore invoke-agent-runtime \
            --agent-runtime-arn ${RUNTIME_ARN} \
            --payload "${PAYLOAD_BASE64}" \
            --region ${AWS_REGION} \
