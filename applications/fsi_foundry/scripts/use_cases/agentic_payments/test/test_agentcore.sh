...
# ============================================================================
# AVA - Agentic Payments AgentCore Test Script
# ============================================================================
# Tests the agentic_payments use case deployed on Bedrock AgentCore Runtime.
# Validates runtime invocation with various payment types.
#
# Usage:
#   ./test_agentcore.sh [use_case_id]
#   FRAMEWORK=strands ./test_agentcore.sh agentic_payments
# ============================================================================

set +e  # Don't exit on error - continue running all tests

# Use case configuration
USE_CASE_ID="${1:-agentic_payments}"
...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}AgentCore Runtime Test - Agentic Payments${NC}"
...
if [ "$STACK_STATUS" = "NOT_FOUND" ]; then
    ...
    echo "  USE_CASE_ID=agentic_payments FRAMEWORK=$FRAMEWORK AWS_REGION=$AWS_REGION ./scripts/deploy/full/deploy_agentcore.sh"
    exit 1
fi
...
# Test 1: Wire Payment Processing (PMT001)
echo -e "${YELLOW}========================================${NC}"
echo -e "${YELLOW}Test 1: Wire Payment Processing (PMT001)${NC}"
echo -e "${YELLOW}========================================${NC}"
echo ""
echo -e "${BLUE}Note: This may take 60-90 seconds as it runs multiple AI agents...${NC}"
echo ""

TEST_PAYLOAD='{"payment_id": "PMT001", "payment_type": "wire"}'
echo "Request:"
echo "${TEST_PAYLOAD}" | jq '.'
echo ""

RESPONSE_FILE="/tmp/agentcore-ap-test1-$$.json"
...
if invoke_agentcore "${TEST_PAYLOAD}" "${RESPONSE_FILE}"; then
    ...
    if echo "$RESPONSE_TEXT" | grep -q "'payment_id': 'PMT001'\|\"payment_id\": \"PMT001\"" && \
       echo "$RESPONSE_TEXT" | grep -q "'transaction_id'\|\"transaction_id\""; then
        echo -e "${GREEN}✓ Wire payment processing PASSED${NC}"
        ((TESTS_PASSED++))
    else
        echo -e "${RED}✗ Wire payment processing FAILED (Invalid response)${NC}"
        ((TESTS_FAILED++))
    fi
else
    echo -e "${RED}✗ Wire payment processing FAILED (Invocation error)${NC}"
    ((TESTS_FAILED++))
fi
rm -f ${RESPONSE_FILE}
echo ""

# Test 2: ACH Payment Processing
echo -e "${YELLOW}========================================${NC}"
echo -e "${YELLOW}Test 2: ACH Payment Processing${NC}"
echo -e "${YELLOW}========================================${NC}"
echo ""

TEST_PAYLOAD='{"payment_id": "PMT001", "payment_type": "ach", "additional_context": "Lower priority batch payment"}'
echo "Request:"
echo "${TEST_PAYLOAD}" | jq '.'
echo ""

RESPONSE_FILE="/tmp/agentcore-ap-test2-$$.json"
...
if invoke_agentcore "${TEST_PAYLOAD}" "${RESPONSE_FILE}"; then
    ...
    RESPONSE_TEXT=$(cat ${RESPONSE_FILE})
    if echo "$RESPONSE_TEXT" | grep -q "'payment_id': 'PMT001'\|\"payment_id\": \"PMT001\""; then
        echo -e "${GREEN}✓ ACH payment processing PASSED${NC}"
        ((TESTS_PASSED++))
    else
        echo -e "${RED}✗ ACH payment processing FAILED (Invalid response)${NC}"
        ((TESTS_FAILED++))
    fi
else
    echo -e "${RED}✗ ACH payment processing FAILED (Invocation error)${NC}"
    ((TESTS_FAILED++))
fi
rm -f ${RESPONSE_FILE}
echo ""

# Test 3: Real-Time Payment Processing
echo -e "${YELLOW}========================================${NC}"
echo -e "${YELLOW}Test 3: Real-Time Payment Processing${NC}"
echo -e "${YELLOW}========================================${NC}"
echo ""

TEST_PAYLOAD='{"payment_id": "PMT001", "payment_type": "real_time", "additional_context": "Urgent payment requiring instant settlement"}'
echo "Request:"
echo "${TEST_PAYLOAD}" | jq '.'
echo ""

RESPONSE_FILE="/tmp/agentcore-ap-test3-$$.json"
...
if invoke_agentcore "${TEST_PAYLOAD}" "${RESPONSE_FILE}"; then
    ...
    RESPONSE_TEXT=$(cat ${RESPONSE_FILE})
    if echo "$RESPONSE_TEXT" | grep -q "'payment_id': 'PMT001'\|\"payment_id\": \"PMT001\""; then
        echo -e "${GREEN}✓ Real-time payment processing PASSED${NC}"
        ((TESTS_PASSED++))
    else
        echo -e "${RED}✗ Real-time payment processing FAILED (Invalid response)${NC}"
        ((TESTS_FAILED++))
    fi
else
    echo -e "${RED}✗ Real-time payment processing FAILED (Invocation error)${NC}"
    ((TESTS_FAILED++))
fi
rm -f ${RESPONSE_FILE}
echo ""

# Test 4: Invalid Payment ID
echo -e "${YELLOW}========================================${NC}"
echo -e "${YELLOW}Test 4: Invalid Payment ID${NC}"
echo -e "${YELLOW}========================================${NC}"
echo ""

TEST_PAYLOAD='{"payment_id": "INVALID999", "payment_type": "wire"}'
echo "Request:"
echo "${TEST_PAYLOAD}" | jq '.'
echo ""

RESPONSE_FILE="/tmp/agentcore-ap-test4-$$.json"

if invoke_agentcore "${TEST_PAYLOAD}" "${RESPONSE_FILE}"; then
    RESPONSE_TEXT=$(cat ${RESPONSE_FILE})
    if echo "$RESPONSE_TEXT" | grep -qi "error\|not found\|No data found"; then
        echo -e "${GREEN}✓ Invalid payment handling PASSED${NC}"
        ((TESTS_PASSED++))
    else
        echo -e "${YELLOW}⚠ Invalid payment handling PASSED (graceful handling)${NC}"
        ((TESTS_PASSED++))
    fi
else
    echo -e "${GREEN}✓ Invalid payment handling PASSED (error returned)${NC}"
    ((TESTS_PASSED++))
fi
rm -f ${RESPONSE_FILE}
echo ""
...
echo -e "${YELLOW}========================================${NC}"
echo ""
echo -e "${BLUE}Note: Running 5 concurrent payment requests...${NC}"
echo ""
...
for i in {1..5}; do
    (
        PAYLOAD='{"payment_id": "PMT001", "payment_type": "ach"}'
        PAYLOAD_BASE64=$(echo -n "${PAYLOAD}" | base64)
        TEMP_FILE="/tmp/agentcore-ap-load-$i-$$.json"
        aws bedrock-agentcore invoke-agent-runtime \
            --agent-runtime-arn ${RUNTIME_ARN} \
            --payload "${PAYLOAD_BASE64}" \
            --region ${AWS_REGION} \
