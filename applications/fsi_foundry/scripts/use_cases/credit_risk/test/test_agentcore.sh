...
# ============================================================================
# AVA - Credit Risk AgentCore Test Script
# ============================================================================
# Tests the credit_risk use case deployed on Bedrock AgentCore Runtime.
# Validates runtime invocation with various inquiry types.
#
# Usage:
#   ./test_agentcore.sh [use_case_id]
#   FRAMEWORK=strands ./test_agentcore.sh credit_risk
# ============================================================================

set +e  # Don't exit on error - continue running all tests

# Use case configuration
USE_CASE_ID="${1:-credit_risk}"
...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}AgentCore Runtime Test - Credit Risk${NC}"
...
if [ "$STACK_STATUS" = "NOT_FOUND" ]; then
    ...
    echo "  USE_CASE_ID=credit_risk FRAMEWORK=$FRAMEWORK AWS_REGION=$AWS_REGION ./scripts/main/deploy.sh"
    exit 1
fi
...
# Test 1: Full Credit Risk Assessment (CUST001)
echo -e "${YELLOW}========================================${NC}"
echo -e "${YELLOW}Test 1: Full Credit Risk Assessment (CUST001)${NC}"
echo -e "${YELLOW}========================================${NC}"
echo ""
...
if invoke_agentcore "${TEST_PAYLOAD}" "${RESPONSE_FILE}"; then
    ...
    if echo "$RESPONSE_TEXT" | grep -q "'customer_id': 'CUST001'\|\"customer_id\": \"CUST001\"" && \
       echo "$RESPONSE_TEXT" | grep -q "'assessment_id'\|\"assessment_id\""; then
...
# Test 2: Financial Analysis (CUST001)
echo -e "${YELLOW}========================================${NC}"
echo -e "${YELLOW}Test 2: Financial Analysis (CUST001)${NC}"
echo -e "${YELLOW}========================================${NC}"
echo ""

TEST_PAYLOAD='{"customer_id": "CUST001", "assessment_type": "financial_analysis", "additional_context": "What are my account details?"}'
echo "Request:"
echo "${TEST_PAYLOAD}" | jq '.'
echo ""
...
# Test 3: Risk Scoring (CUST001)
echo -e "${YELLOW}========================================${NC}"
echo -e "${YELLOW}Test 3: Risk Scoring (CUST001)${NC}"
echo -e "${YELLOW}========================================${NC}"
echo ""

TEST_PAYLOAD='{"customer_id": "CUST001", "assessment_type": "risk_scoring", "additional_context": "Dispute TXN003"}'
echo "Request:"
echo "${TEST_PAYLOAD}" | jq '.'
echo ""
...
echo -e "${YELLOW}========================================${NC}"
echo ""
echo -e "${BLUE}Note: Running 5 concurrent financial analysis requests...${NC}"
echo ""
...
for i in {1..5}; do
    (
        PAYLOAD='{"customer_id": "CUST001", "assessment_type": "financial_analysis"}'
