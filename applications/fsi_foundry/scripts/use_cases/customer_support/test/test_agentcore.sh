...
# ============================================================================
# AVA - Customer Support AgentCore Test Script
# ============================================================================
# Tests the customer_support use case deployed on Bedrock AgentCore Runtime.
# Validates runtime invocation with various inquiry types.
#
# Usage:
#   ./test_agentcore.sh [use_case_id]
#   FRAMEWORK=strands ./test_agentcore.sh customer_support
# ============================================================================

set +e  # Don't exit on error - continue running all tests

# Use case configuration
USE_CASE_ID="${1:-customer_support}"
...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}AgentCore Runtime Test - Customer Support${NC}"
...
if [ "$STACK_STATUS" = "NOT_FOUND" ]; then
    ...
    echo "  USE_CASE_ID=customer_support FRAMEWORK=$FRAMEWORK AWS_REGION=$AWS_REGION ./scripts/main/deploy.sh"
    exit 1
fi
...
# Test 1: Full Support Assessment (CUST001)
echo -e "${YELLOW}========================================${NC}"
echo -e "${YELLOW}Test 1: Full Support Assessment (CUST001)${NC}"
echo -e "${YELLOW}========================================${NC}"
echo ""
echo -e "${BLUE}Note: This may take 60-90 seconds as it runs multiple AI agents...${NC}"
echo ""

TEST_PAYLOAD='{"customer_id": "CUST001", "ticket_type": "full"}'
echo "Request:"
...
if invoke_agentcore "${TEST_PAYLOAD}" "${RESPONSE_FILE}"; then
    ...
    if echo "$RESPONSE_TEXT" | grep -q "'customer_id': 'CUST001'\|\"customer_id\": \"CUST001\"" && \
       echo "$RESPONSE_TEXT" | grep -q "'ticket_id'\|\"ticket_id\""; then
...
# Test 2: General Ticket (CUST001)
echo -e "${YELLOW}========================================${NC}"
echo -e "${YELLOW}Test 2: General Ticket (CUST001)${NC}"
echo -e "${YELLOW}========================================${NC}"
echo ""

TEST_PAYLOAD='{"customer_id": "CUST001", "ticket_type": "general", "additional_context": "What are my account details?"}'
echo "Request:"
echo "${TEST_PAYLOAD}" | jq '.'
echo ""
...
# Test 3: Billing Ticket (CUST001)
echo -e "${YELLOW}========================================${NC}"
echo -e "${YELLOW}Test 3: Billing Ticket (CUST001)${NC}"
echo -e "${YELLOW}========================================${NC}"
echo ""

TEST_PAYLOAD='{"customer_id": "CUST001", "ticket_type": "billing", "additional_context": "Dispute TXN003"}'
echo "Request:"
echo "${TEST_PAYLOAD}" | jq '.'
echo ""
...
TEST_PAYLOAD='{"customer_id": "INVALID999", "ticket_type": "full"}'
echo "Request:"
echo "${TEST_PAYLOAD}" | jq '.'
echo ""
...
echo -e "${YELLOW}========================================${NC}"
echo ""
echo -e "${BLUE}Note: Running 5 concurrent general tickets...${NC}"
echo ""
...
for i in {1..5}; do
    (
        PAYLOAD='{"customer_id": "CUST001", "ticket_type": "general"}'
