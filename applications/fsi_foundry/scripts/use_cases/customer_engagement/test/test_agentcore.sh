...
# ============================================================================
# AVA - Customer Engagement AgentCore Test Script
# ============================================================================
# Tests the customer_engagement use case deployed on Bedrock AgentCore Runtime.
# Validates runtime invocation with various assessment types.
#
# Usage:
#   ./test_agentcore.sh [use_case_id]
#   FRAMEWORK=strands ./test_agentcore.sh customer_engagement
# ============================================================================

set +e  # Don't exit on error - continue running all tests

# Use case configuration
USE_CASE_ID="${1:-customer_engagement}"
...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}AgentCore Runtime Test - Customer Engagement${NC}"
...
if [ "$STACK_STATUS" = "NOT_FOUND" ]; then
    ...
    echo "  USE_CASE_ID=customer_engagement FRAMEWORK=$FRAMEWORK AWS_REGION=$AWS_REGION ./scripts/deploy/full/deploy_agentcore.sh"
    exit 1
fi
...
# Test 1: Full Engagement Assessment (POLICY001)
echo -e "${YELLOW}========================================${NC}"
echo -e "${YELLOW}Test 1: Full Engagement Assessment (POLICY001)${NC}"
echo -e "${YELLOW}========================================${NC}"
echo ""
echo -e "${BLUE}Note: This may take 60-90 seconds as it runs multiple AI agents...${NC}"
echo ""

TEST_PAYLOAD='{"customer_id": "POLICY001", "assessment_type": "full"}'
echo "Request:"
echo "${TEST_PAYLOAD}" | jq '.'
echo ""

RESPONSE_FILE="/tmp/agentcore-ce-test1-$$.json"
...
if invoke_agentcore "${TEST_PAYLOAD}" "${RESPONSE_FILE}"; then
    ...
    if echo "$RESPONSE_TEXT" | grep -q "'customer_id': 'POLICY001'\|\"customer_id\": \"POLICY001\"" && \
       echo "$RESPONSE_TEXT" | grep -q "'churn_prediction'\|\"churn_prediction\"" && \
       echo "$RESPONSE_TEXT" | grep -q "'outreach_plan'\|\"outreach_plan\"" && \
       echo "$RESPONSE_TEXT" | grep -q "'policy_recommendations'\|\"policy_recommendations\""; then
        echo -e "${GREEN}✓ Full engagement assessment PASSED${NC}"
        ((TESTS_PASSED++))
    else
        echo -e "${RED}✗ Full engagement assessment FAILED (Invalid response)${NC}"
        ((TESTS_FAILED++))
    fi
else
    echo -e "${RED}✗ Full engagement assessment FAILED (Invocation error)${NC}"
    ((TESTS_FAILED++))
fi
rm -f ${RESPONSE_FILE}
echo ""

# Test 2: Churn Prediction Only (POLICY001)
echo -e "${YELLOW}========================================${NC}"
echo -e "${YELLOW}Test 2: Churn Prediction Only (POLICY001)${NC}"
echo -e "${YELLOW}========================================${NC}"
echo ""

TEST_PAYLOAD='{"customer_id": "POLICY001", "assessment_type": "churn_only"}'
echo "Request:"
echo "${TEST_PAYLOAD}" | jq '.'
echo ""

RESPONSE_FILE="/tmp/agentcore-ce-test2-$$.json"
...
if invoke_agentcore "${TEST_PAYLOAD}" "${RESPONSE_FILE}"; then
    ...
    RESPONSE_TEXT=$(cat ${RESPONSE_FILE})
    if echo "$RESPONSE_TEXT" | grep -q "'churn_prediction'\|\"churn_prediction\"" && \
       echo "$RESPONSE_TEXT" | grep -q "'risk_level'\|\"risk_level\""; then
        echo -e "${GREEN}✓ Churn prediction PASSED${NC}"
        ((TESTS_PASSED++))
    else
        echo -e "${RED}✗ Churn prediction FAILED (Invalid response)${NC}"
        ((TESTS_FAILED++))
    fi
else
    echo -e "${RED}✗ Churn prediction FAILED (Invocation error)${NC}"
    ((TESTS_FAILED++))
fi
rm -f ${RESPONSE_FILE}
echo ""

# Test 3: Outreach Planning Only (POLICY001)
echo -e "${YELLOW}========================================${NC}"
echo -e "${YELLOW}Test 3: Outreach Planning Only (POLICY001)${NC}"
echo -e "${YELLOW}========================================${NC}"
echo ""

TEST_PAYLOAD='{"customer_id": "POLICY001", "assessment_type": "outreach_only"}'
echo "Request:"
echo "${TEST_PAYLOAD}" | jq '.'
echo ""

RESPONSE_FILE="/tmp/agentcore-ce-test3-$$.json"
...
if invoke_agentcore "${TEST_PAYLOAD}" "${RESPONSE_FILE}"; then
    ...
    RESPONSE_TEXT=$(cat ${RESPONSE_FILE})
    if echo "$RESPONSE_TEXT" | grep -q "'outreach_plan'\|\"outreach_plan\"" && \
       echo "$RESPONSE_TEXT" | grep -q "'recommended_channel'\|\"recommended_channel\""; then
        echo -e "${GREEN}✓ Outreach planning PASSED${NC}"
        ((TESTS_PASSED++))
    else
        echo -e "${RED}✗ Outreach planning FAILED (Invalid response)${NC}"
        ((TESTS_FAILED++))
    fi
else
    echo -e "${RED}✗ Outreach planning FAILED (Invocation error)${NC}"
    ((TESTS_FAILED++))
fi
rm -f ${RESPONSE_FILE}
echo ""

# Test 4: Policy Optimization Only (POLICY001)
echo -e "${YELLOW}========================================${NC}"
echo -e "${YELLOW}Test 4: Policy Optimization Only (POLICY001)${NC}"
echo -e "${YELLOW}========================================${NC}"
echo ""

TEST_PAYLOAD='{"customer_id": "POLICY001", "assessment_type": "policy_only"}'
echo "Request:"
echo "${TEST_PAYLOAD}" | jq '.'
echo ""

RESPONSE_FILE="/tmp/agentcore-ce-test4-$$.json"
...
if invoke_agentcore "${TEST_PAYLOAD}" "${RESPONSE_FILE}"; then
    ...
    RESPONSE_TEXT=$(cat ${RESPONSE_FILE})
    if echo "$RESPONSE_TEXT" | grep -q "'policy_recommendations'\|\"policy_recommendations\"" && \
       echo "$RESPONSE_TEXT" | grep -q "'recommended_actions'\|\"recommended_actions\""; then
        echo -e "${GREEN}✓ Policy optimization PASSED${NC}"
        ((TESTS_PASSED++))
    else
        echo -e "${RED}✗ Policy optimization FAILED (Invalid response)${NC}"
        ((TESTS_FAILED++))
    fi
else
    echo -e "${RED}✗ Policy optimization FAILED (Invocation error)${NC}"
    ((TESTS_FAILED++))
fi
rm -f ${RESPONSE_FILE}
echo ""
...
echo -e "${YELLOW}========================================${NC}"
echo -e "${YELLOW}Test 5: Invalid Customer ID${NC}"
echo -e "${YELLOW}========================================${NC}"
echo ""
...
RESPONSE_FILE="/tmp/agentcore-ce-test5-$$.json"
...
echo -e "${YELLOW}========================================${NC}"
echo -e "${YELLOW}Test 6: Load Test (5 concurrent)${NC}"
echo -e "${YELLOW}========================================${NC}"
echo ""
echo -e "${BLUE}Note: Running 5 concurrent churn assessments...${NC}"
echo ""
...
for i in {1..5}; do
    (
        PAYLOAD='{"customer_id": "POLICY001", "assessment_type": "churn_only"}'
        PAYLOAD_BASE64=$(echo -n "${PAYLOAD}" | base64)
        TEMP_FILE="/tmp/agentcore-ce-load-$i-$$.json"
        aws bedrock-agentcore invoke-agent-runtime \
            --agent-runtime-arn ${RUNTIME_ARN} \
            --payload "${PAYLOAD_BASE64}" \
            --region ${AWS_REGION} \
